"""This file is used to display a window for communication with the user"""
...
from model import LogisticRegression, model  # import the trained logistic regression model
...
class Window(QWidget):
    def __init__(self):
        ...
        # Image display area (graphic label)
        ...
        # Input fields for entering pressure values
        ...
        # Button to demonstrate example calculations
    ...
    """Function to insert example data — you can expand it"""
    ...
    def button_examp_click(self):
        ...
        self.example_index = (self.example_index + 1) % len(self.example_data)   # cycling through example values

    """Perform prediction calculations and update the graphical interface"""

    def machine_calculate(self, x1, x2, x3, x4):
        try:
            X_new = np.array([[x1, x2, x3, x4]])  # New data as array
            X_new = self.normalization(X_new)   # Normalize and extract from array
            y_pred = model.predict(X_new)   # Model prediction
            print(X_new)
            print("Predict:", y_pred[0])
            if y_pred[0] == True:
                # Image with rotation in case of "robot on stairs"
